...
class QuoteCardController extends GetxController {
  ...
  RxList imageList = ['bridge','cloud','door','mountain','sun','tree','up'].obs;
  ...
  Future<void> saveFavoriteQuote(Map item) async {
    ...
    List<String> encodedList = favoriteQuotes.map((item) => jsonEncode(item)).toList();
  ...
  loadInnerData() async{
    ...
    final List<String> encodedList = prefs.getStringList('favoriteQuotes') ?? [];
  ...
  getRandomQuotes() {
    ...
    while (selectedIndexes.length < 5 && selectedIndexes.length < quoteList.length) {
